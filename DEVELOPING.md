...
3. Run the checks:
   ...
   cargo check --all-targets --features bench-pbnsolve    # off by default, so it rots quietly
   ```
...
Pushing anything under `puzzles/` triggers `.github/workflows/puzzle_archive.yml`, which rebuilds
`puzzles.zip` and publishes it to GitHub Pages at
https://paulstansifer.github.io/number-loom/puzzles.zip. That's what the in-app Library button
fetches via `import::load_library`. Give it a couple of minutes (plus a CDN cache
of about ten) before a new puzzle shows up in the gallery.
