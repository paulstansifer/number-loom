...
5. `trunk build --release`, then `(cd dist; zip -r dist.zip .)`, then upload `dist/dist.zip` to
   ...
   And maybe check that the gallery works right from there
6. `cargo publish`.
...
the `puzzles.zip` asset attached to the `latest` release; however, I haven't figured out a way to access this yet.
